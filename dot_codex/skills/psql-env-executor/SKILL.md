...
```bash
/home/leoc/.codex/skills/psql-env-executor/scripts/exec_psql_env.sh \
  --env staging \
  --service back \
  --sql 'SELECT 1;'
```
...
```bash
cat /tmp/query.sql | /home/leoc/.codex/skills/psql-env-executor/scripts/exec_psql_env.sh --env local
```
