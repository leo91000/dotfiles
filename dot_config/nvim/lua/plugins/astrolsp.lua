    ...
    vim.fn.jobstart("npm i -g @vue/typescript-plugin", {
      on_exit = function(_job_id, exit_code)
        if exit_code == 0 then
          ...
          vim.notify("Failed to install @vue/typescript-plugin. Exit code: " .. exit_code, vim.log.levels.ERROR)
        end
      end,
...
local function has_eslint_config(root_dir)
  ...
  for _, filename in ipairs(eslint_files) do
    if util.path.exists(util.path.join(root_dir, filename)) then return true end
  end
...
return {
  ...
  opts = {
    ...
    -- Default configurations for hover and signature help (new in v5)
    defaults = {
      -- hover = {},
      -- signature_help = {},
    },
